...
// 返回当前线程的中断等级,并把中断等级置为可中断
enum intr_level
intr_enable (void) 
{
  enum intr_level old_level = intr_get_level ();
  // ASSERT断言这个中断不是外中断（IO等， 也称为硬中断）
  // 而是操作系统正常线程切换流程里的内中断（也称为软中断）。
  ASSERT (!intr_context ());
...
intr_disable (void) 
{
  ...
  /* Disable interrupts by clearing the interrupt flag.
     ...
     Hardware Interrupts". */
  // 这个asm volatile是在C语言中内嵌了汇编语言，调用了CLI指令，CLI指令不是command line interface, 而是clear interrupt.
  // 调用汇编指令将标志寄存器的IF（interrupt flag）位置为0, IF=0时将不响应可屏蔽中断。
